...
- **The same component decides how eagerly a picture loads, and it reads that
  ...
  loads eagerly while the sidebar and every admin row stay lazy. One
  unconditional `loading="lazy"` deferred all three, and the one image a reader
  came to look at then paints after the layout around it, so an upload that has
  already succeeded reads as not having taken. Nothing errors, and a fast machine
  never shows it.
