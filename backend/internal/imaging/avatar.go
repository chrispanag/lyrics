...
// application stores: a centered square, re-encoded as JPEG.
//
...
//
// Cropping here rather than only in the browser is what makes "a stored picture
// is square" true of every row instead of every row a well-behaved client wrote.
// This is the one layer no client can skip, so every consumer — the app's
// avatars today, an email or an OG image later — can rely on it.
package imaging
...
const (
	...
	// bitmap this package will allocate. The browser sends 256px squares.
	MaxDimension = 1024
...
// Normalize validates an uploaded image and returns a square JPEG of it,
// together with the content type to store it under.
...
// square center-crops an image, flattening transparency onto white if it has
// any.
func square(src image.Image) image.Image {
	...
	if opaque, ok := src.(interface{ Opaque() bool }); !ok || !opaque.Opaque() {
		...
		draw.Draw(dst, dst.Bounds(), src, crop.Min, draw.Over)
		return dst
	}
